...
echo
saId=$(az storage account create -n $saName -g $RESOURCE_GROUP --enable-hierarchical-namespace -o tsv --query id)

# create schema registry
echo "Creating schema registry..."
srId=$(az iot ops schema registry create -n $srName -g $RESOURCE_GROUP --registry-namespace $srName --sa-resource-id $saId -o tsv --query id)
